...
async def exec_code(run_id: str, req: PushCodeRequest, request: Request):
    ...
    if not ws.exists():
        raise HTTPException(404, "Workspace not found")
    # Always inject DATA_PATH/DATA_SEP/TIME_BUDGET header — strip any existing
    # definitions first to avoid NameError when user edits code in the editor.
    import re as _re
    code = req.code
    # Strip any existing header lines so we don't double-define
    code = _re.sub(r'^(DATA_PATH|DATA_SEP|TIME_BUDGET)\s*=\s*.+\n?', '', code, flags=_re.MULTILINE)
...
async def debug_list_models(request: Request):
    user = _get_user(_token_from_request(request))
    if not user:
        raise HTTPException(401, "Authentication required")
...
async def _predict_file_core(run_id: str, contents: bytes, run: dict) -> dict:
    ...
    try:
        ...
        _pred_mean = float(preds.mean()) if len(preds) > 0 else 0
        ...
        _meta_log = any(kw in _what or kw in _model_str
                        for kw in ("log1p", "log_", "log transform", "logtransform", "log("))

        # Heuristic 2: all preds tightly clustered in [0, 20] — unambiguous log-space signature.
        # Real-world continuous regression targets almost never live entirely in a 0-20 band.
        # (Price/revenue data would be 100s-100000s; classification probability is 0-1 but
        #  those models don't use log1p transform.)
        _range_log = (_pred_min >= 0 and _pred_max < 20)

        # Heuristic 3: check training RMSE >> pred range (works even if heuristic 2 misses)
        _rmse = float(_best.get("rmse") or _best.get("test_rmse") or 0)
        _rmse_log = (_pred_min >= 0 and _pred_max < 50 and _rmse > (_pred_max * 5 + 1))

        _should_expm1 = _meta_log or _range_log or _rmse_log
    except Exception:
        pass
    if _should_expm1:
        preds = _np.expm1(preds)
        print(f"[predict-file] Applied expm1 inverse transform (log-space model detected, "
              f"pred range was [{_pred_min:.2f}, {_pred_max:.2f}])", flush=True)
...
async def view_shared(share_id: str):
    ...
    # Build a standalone HTML page — every user-controlled value is escaped
    experiments_html = ""
    for h in history:
        if h.get("success"):
            ...
            experiments_html += (
                f'<div class="exp {cls}">'
                f'<span class="num">#{int(h.get("num", 0))}</span>'
                f'<span class="model">{_e(str(h.get("model","?")))}</span>'
                f'<span class="val">{_e(str(h.get("metric_name","metric")))}={float(h.get("metric_val",0)):.4f}</span>'
                f'</div>'
            )

    score = _e(str(diag.get("yc_readiness_score", "N/A")))
    grade = _e(str(diag.get("yc_grade", "N/A")))
    brief = _e(str(data.get("executive_brief", "")))
    report = _e(str(data.get("report", ""))[:3000])
    model_name = _e(str(best.get("model", "ML Model")))
    filename = _e(str(data.get("filename", "")))
    metric_name = _e(str(best.get("metric_name", "metric")).upper())
    task = _e(str(obj.get("task", "N/A")))
    target = _e(str(obj.get("target", "N/A")))

    shared_html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><title>19Labs Results — {model_name}</title>
...
<h1>{model_name}</h1>
<div class="sub">Generated by 19Labs Autonomous ML Research · {filename}</div>
<div class="card">
<div class="metric">{metric_name} = {float(best.get('metric_val',0)):.4f}</div>
<div class="row"><span class="label">Task</span><span class="value">{task}</span></div>
...
<h2>Experiments</h2>
{experiments_html}
{f'<h2>Report</h2><div class="report">{report}</div>' if report else ''}
<div class="footer">Powered by <a href="/">19Labs</a> · Autonomous ML Research</div>
</body></html>"""

    return HTMLResponse(shared_html)
