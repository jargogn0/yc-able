...
def apply_code_guardrails(code: str) -> tuple[str, list[str]]:
    ...
    _SAVE_TAIL = (
        ...
        "\n"
        "# ── 19Labs: auto-generate submission.csv if test file present ──\n"
        "if not _19pl.Path('submission.csv').exists():\n"
        "    import os as _19os\n"
        "    _19_data_dir = _19os.path.dirname(globals().get('DATA_PATH','')) or '.'\n"
        "    _19_test_f = None\n"
        "    for _19_fn in _19os.listdir(_19_data_dir):\n"
        "        if ('test' in _19_fn.lower() and _19_fn.lower().endswith('.csv')\n"
        "                and 'train' not in _19_fn.lower() and 'sample' not in _19_fn.lower()):\n"
        "            _19_test_f = _19os.path.join(_19_data_dir, _19_fn); break\n"
        "    if _19_test_f:\n"
        "        try:\n"
        "            import pandas as _19pd\n"
        "            _19_test_df = _19pd.read_csv(_19_test_f)\n"
        "            _19_mp = 'model.pkl' if _19pl.Path('model.pkl').exists() else 'best_model.pkl'\n"
        "            _19_mo = _19jl.load(_19_mp)\n"
        "            # Align columns using saved feature schema\n"
        "            _19_feat = _19jl.load('feature_cols.pkl') if _19pl.Path('feature_cols.pkl').exists() else None\n"
        "            if _19_feat:\n"
